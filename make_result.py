...
def main(fang_dir=None, fu_dir=None, validate_xls=None, progress_callback=None, args=None):
    ...
    if fang_dir is not None and fang_dir != "":
        project_fang_list = make_fang_result(fang_dir=Path(fang_dir),progress_callback=progress_callback)
    if fu_dir is not None and fu_dir != "":
        project_fu_list = make_fu_result(fu_dir=Path(fu_dir),progress_callback=progress_callback)
